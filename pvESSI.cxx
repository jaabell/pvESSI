...
void pvESSI::Build_Node_Attributes(vtkSmartPointer<vtkUnstructuredGrid> Node_Mesh, int Current_Time){
	...
	this->Set_Meta_Array (Meta_Array_Map["Element_Tag"]);	
 	...
 	float *displacements = new float[3];
	...
	while(++index_i < Number_of_Nodes){
	    ...
	    HDF5_Read_INT_Array_Data(id_Node_Map,
		                         ...
		                         count1,
		                         NULL,
		                         &Int_Variable_1); // Node_Map_Node_Number

	    Node_Tag -> InsertValue((int) index_i,Int_Variable_1);

	    index_j = (hsize_t) Int_Variable_1;

	    HDF5_Read_INT_Array_Data(id_Index_to_Generalized_Displacements,
		                          1,
		                         dims1_out,
		                         &index_j,
		                         NULL,
		                         count1,
		                         NULL,
		                         &Int_Variable_1); // Index_to_Generalized_Displacements

	    offset2[0] = (hsize_t)Int_Variable_1;  offset2[1] = Current_Time;
	    count2 [0] = 3;  count2 [1] = 1; 
	    dims1_out[0] =3;

	    HDF5_Read_FLOAT_Array_Data(id_Generalized_Displacements,
		                          2,
		                         dims1_out,
		                         offset2,
		                         NULL,
		                         count2,
		                         NULL,
		                         displacements); // Generalized_Displacements


	    Generalized_Displacements->InsertTypedTuple((int) index_i,displacements);
	...
	while(++index_i < Number_of_Elements){
	    ...
	    HDF5_Read_INT_Array_Data(id_Element_Map,
		                         ...
		                         count1,
		                         NULL,
		                         &Int_Variable_1); // Element_Map_Element_Number

	    Element_Tag -> InsertValue((int) index_i,Int_Variable_1);

	    index_j = (hsize_t) Int_Variable_1;

	    HDF5_Read_INT_Array_Data(id_Material_tags,
		                          1,
		                         dims1_out,
		                         &index_j,
		                         NULL,
	    ...
	    Material_Tag -> InsertValue((int) index_i,Int_Variable_1);
...
void pvESSI::Build_Gauss_Attributes(vtkSmartPointer<vtkUnstructuredGrid> Gauss_Mesh, int Current_Time){
	...
	float *Gauss_Outputs = new float[18];
	...
	while(++index_i < Number_of_Elements){
	    ...
	    HDF5_Read_INT_Array_Data(id_Number_of_Gauss_Points,
		                         ...
		                         count1,
		                         NULL,
		                         &Int_Variable_1); // Element_Number_of_Gauss_Points
		...
		while(++index_j < Int_Variable_1){

			index++;

		    offset2[0] = (hsize_t)(Int_Variable_2) + (18*index_j);  offset2[1] = Current_Time;
		    count2 [0] = 18;  count2 [1] = 1; 
		    dims1_out[0] =18;


		    HDF5_Read_FLOAT_Array_Data(id_Outputs,
			                          1,
			                         dims1_out,
			                         offset2,
			                         NULL,
			                         count2,
			                         NULL,
			                         Gauss_Outputs); // Element_Gauss_Outputs
			...
			Elastic_Strain->InsertTypedTuple (index,El_Strain_Tuple);
			Plastic_Strain->InsertTypedTuple (index,Pl_Strain_Tuple);
			Stress->InsertTypedTuple (index,Stress_Tuple);
		}
...
void pvESSI::Get_Gauss_Mesh(vtkSmartPointer<vtkUnstructuredGrid> Gauss_Mesh){
	...
	while(++index_i < Number_of_Elements){
	    ...
	    HDF5_Read_INT_Array_Data(id_Index_to_Gauss_Point_Coordinates,
		                          1,
		                         dims1_out,
		                         &index_j,
		                         NULL,
		                         count1,
		                         NULL,
		                         &Int_Variable_2); // Index_to_Gauss_Point_Coordinates
		...
		while(++index_j < Int_Variable_1){
			...
			index_k = (hsize_t)(Int_Variable_2) + 3*index_j;

		    HDF5_Read_FLOAT_Array_Data(id_Gauss_Point_Coordinates,
			                          1,
			                         dims1_out,
			                         &index_k,
			                         NULL,
			                         count1,
			                         NULL,
			                         coordinates); // Gauss_Point_Coordinates

		    onevertex = ++this->Number_of_Gauss_Nodes;
...
void pvESSI::Build_Stress_Field_At_Nodes(vtkSmartPointer<vtkUnstructuredGrid> Node_Mesh, int Node_Mesh_Current_Time){
	...
	if(Int_Variable_1==-1){
	    ...
	    dims3[1] = dims3[1]<Number_Of_Time_Steps?(dims3[1]+1):Number_Of_Time_Steps;
	    dims3[2] = 18;
	...
	// 	DataSpace = H5Dget_space(DataSet);
	//     D3_offset[0]=0;                     D3_offset[1]=Int_Variable_1; 	                 D3_offset[2]=0;
	//     D3_count[0] =this->Number_of_Nodes;  D3_count[1]=1;							         D3_count[2] =18;
	//     count[0]    =this->Number_of_Nodes;     count[1]=18;                                 MemSpace = H5Screate_simple(2,count,NULL);
	// 	status = H5Sselect_hyperslab(DataSpace,H5S_SELECT_SET,D3_offset,NULL,D3_count,NULL);
...
void pvESSI::Build_Maps(){
	...
	DataSpace = H5Screate_simple(1, dims1_out, NULL);
	id_Whether_Stress_Strain_Build = H5Dcreate(id_File,"Field_at_Nodes/Whether_Stress_Strain_Build",H5T_STD_I32BE,DataSpace,H5P_DEFAULT,H5P_DEFAULT, H5P_DEFAULT);
	H5Sclose(DataSpace);
